...
class CP2800(SerialInstrument):
    def __init__(self, name="", address='COM6', enabled=True, baudrate=9600, timeout=0.25):
    ...
    def parse_message(self, bitstring):
        ...
        """
        preamble_len = 7
        intlist = [bitstring[i] for i in range(len(bitstring))]
        new_intlist = list()
        was_seven = False
    ...
    def get_compressor_status(self):
        ...
        answer = self.parse_message(self.ser.read(1028))
    ...
    def get_compressor_temperatures(self):
        ...
        """
        temperatures = list()
        temperatures.append(self.get_input_water_temperature())
        temperatures.append(self.get_output_water_temperature())
        temperatures.append(self.get_helium_temperature())
        temperatures.append(self.get_oil_temperature())
        return temperatures
    ...
    def stop_compressor(self):
        ...
        NOTE: Code in manual above is wrong for the new models of the CP2800, the write byte must be "1", not "0" as in the manual
        """
...
if __name__ == "__main__":
    cp = CP2800(address='COM6', baudrate=9600, timeout=0.1)
